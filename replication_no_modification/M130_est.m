diary M130_est_tong_distance_demand.txt
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% flight, with an period with at least more than 3000 miles
% flying time (5h+)
load M100_db1b  

% Create a histogram
figure;
histogram(dist, 'Normalization', 'probability', 'EdgeColor', 'white');
title('Distribution of Distance amongst the flights');
xlabel('Miles');
ylabel('Probability');

% here we have the distribution of distance, and we will make the adjustment under section IV: 
%%%%%%%%%%%%%%%
clear
format short g
...
% here we only want to keep what the authors defined as a long haul, which
% is 
data = data(LgDist == 1, :);
...
%%%%%% our modification starts here, filtering the data: 
load M100_db1b  
dist_long = data(:, 9);
data = data(dist_long > 3000, :); % filtered
nJ=nJ(:,2);
...
% Demand IV
%%% as we have filtered the data, we have to change the numb of
%%% observations: 

nobs_long = 93439;
IV1=[ones(nobs_long,1),dist,nconn,ndest,ndestend, hub,hubdest,lcc,...   
...
IV2=[ndest,ndestend,hub,hubdest,lcc,...   
    ...
    ones(nobs_long,1).*SmMdDist, dist.*SmMdDist, nconn.*SmMdDist,...
    ones(nobs_long,1).*LgDist, dist.*LgDist, nconn.*LgDist];

%%% since we are only looking at the long distance, we do not need dummies
%%% to control for distance, SmMd ie. 
invA1 = inv(IV1'*IV1);     
invA2 = inv(IV2'*IV2);    
